...
engine = db.create_engine('mysql+mysqlconnector://bi:Corona1427@coronatracker.coehycitad7u.ap-southeast-1.rds.amazonaws.com/coronatracker')
...
data[['confirmed','recovered','deaths']][data.country == 'Mainland China'][data.posted_date == data.posted_date.max()].values[0]

data[data.country == 'Mainland China']

data_cases = data_localized[['state','country',
                   'confirmed', 'recovered','deaths']
                  ][data_localized.posted_date == latest_time].groupby(
                      ['country']).sum().sort_values(by='confirmed', ascending=False)

data_cases

data_cases.reset_index()

country_list = data_cases.reset_index()['country'].unique().tolist()
cases_list = ['confirmed','recovered','deaths']

import plotly.graph_objects as go

fig = go.Figure()

for stat in cases_list:
    fig.add_trace(
        go.Bar(
            name=str(stat),
            y=country_list,
            x=data_cases.reset_index()[stat].tolist(),
            orientation='h'
            )
          )
...
fig.update_layout(
    barmode='group',
    title=f'2019-nCoV Cases by Country as of {latest_time.strftime("%d %B %Y %I:%M %p %Z")}',
    xaxis_type='log')

fig.write_html('bycountry.html', config={
    'responsive': True
})

data_cases_china = data_localized[['state','country',
                   'confirmed', 'recovered','deaths']
                  ][data_localized.posted_date == latest_time
                    ][data_localized.country == 'Mainland China'
                      ].sort_values(by='confirmed', ascending=True)

province_list = data_cases_china['state'].unique().tolist()
cases_list = ['confirmed','recovered','deaths']
...
for stat in cases_list:
    fig.add_trace(
        go.Bar(
            name=str(stat),
            y=province_list[-10:],
            x=data_cases_china[-10:][stat].tolist(),
            orientation='h'
            )
          )
...
fig.update_layout(
    barmode='group',
    title=f'2019-nCoV Cases in China as of {latest_time.strftime("%d %B %Y %I:%M %p %Z")}',)

fig.write_html('china.html', config={
    'responsive': True
})
